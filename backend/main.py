...
def fetch_tri_facilities():
    ...
    for f in raw:
        ...
        tri_id = f.get("TRI_FACILITY_ID")
        if not tri_id or tri_id in seen_ids: continue
        lat = f.get("PREF_LATITUDE")
        lng = f.get("PREF_LONGITUDE")
        try:
        ...
        seen_ids.add(tri_id)
        facilities.append({
            "id": tri_id, "name": f.get("FACILITY_NAME", "Unknown"),
            "address": f.get("STREET_ADDRESS", ""), "city": f.get("CITY_NAME", ""),
            "zip": f.get("ZIP_CODE", ""),
            "industry": f.get("ASGN_FEDERAL_IND", "") or f.get("PARENT_CO_NAME", ""),
...
def fetch_weather():
    """Open-Meteo forecast — no API key, no signup."""
    forecast_url = (
...
@app.get("/api/weather")
def api_weather():
    """Live weather + 7-day forecast from Open-Meteo (no API key)."""
